...
# RM login
r = httpx.post(f"{BASE}/api/v1/auth/login", data={"username": "rm@sentinel.ai", "password": PASS})
...
# Risk login
r = httpx.post(f"{BASE}/api/v1/auth/login", data={"username": "risk@sentinel.ai", "password": PASS})
check("RiskOfficer login", r.status_code == 200)
risk_token = r.json()["access_token"]

# Compliance login
r = httpx.post(f"{BASE}/api/v1/auth/login", data={"username": "compliance@sentinel.ai", "password": PASS})
...
section("3. PLAYBOOKS")
r = httpx.get(f"{BASE}/api/v1/playbooks/", headers=rm_h)
...
r = httpx.post(f"{BASE}/api/v1/scenarios/", json=scenario_payload, headers=risk_h)
...
r = httpx.get(f"{BASE}/api/v1/scenarios/", headers=rm_h)
...
r = httpx.get(f"{BASE}/api/v1/approvals/pending", headers=risk_h)
...
r = httpx.get(f"{BASE}/api/v1/approvals/pending", headers=comp_h)
...
r = httpx.post(f"{BASE}/api/v1/playbooks/", json={"name":"test","trigger_conditions":{},"actions":[],"compliance_rules":{}}, headers=rm_h)
check("RM blocked from creating playbooks", r.status_code == 403, f"HTTP {r.status_code}")

# RiskOfficer can create playbooks
r = httpx.post(f"{BASE}/api/v1/playbooks/", json={"name":"Test Playbook E2E","description":"E2E test","trigger_conditions":{"type":"test"},"actions":[],"compliance_rules":{}}, headers=risk_h)
